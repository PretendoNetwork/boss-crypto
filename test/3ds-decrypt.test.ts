import { describe, it, expect } from 'vitest';
import { decrypt3DS } from '@pretendonetwork/boss-crypto';

const BOSS_3DS_AES_KEY = process.env.BOSS_3DS_AES_KEY!;

describe('decrypt3DS', () => {
	it('should correctly decrypt 3DS BOSS data', () => {
		const expected = {
			hash_type: 2,
			serial_number: 1692231927n,
			iv: Buffer.from('00000000000000000000000000000001', 'hex'),
			flags: 0n,
			content_header_hash: Buffer.from('5cd5d8198b7ce64edc796c147f333a76c3714269066504c9232f28cabe4306db', 'hex'),
			content_header_hash_signature: Buffer.from('', 'hex'),
			payload_contents_length: 1,
			payload_content_header_hash: Buffer.from('e3403f8fcd420e9fa3e8524779edfa5b08ba316be5d377e42e71e6cdcdf03207', 'hex'),
			payload_content_header_hash_signature: Buffer.from('', 'hex'),
			program_id: 1125968626460928n,
			content_datatype: 65537,
			ns_data_id: 36,
			content: Buffer.from('48656c6c6f20576f726c64', 'hex')
		};

		const encrypted = Buffer.from([
			
		]);

		const decrypted = decrypt3DS(encrypted, BOSS_3DS_AES_KEY);

		expect(decrypted.hash_type).toBe(expected.hash_type);
		expect(decrypted.serial_number).toBe(expected.serial_number);
		expect(decrypted.iv.equals(expected.iv)).toBe(true);
		expect(decrypted.flags).toBe(expected.flags);
		expect(decrypted.content_header_hash.equals(expected.content_header_hash)).toBe(true);
		expect(decrypted.content_header_hash_signature.equals(expected.content_header_hash_signature)).toBe(true);
		expect(decrypted.payload_contents.length).toBe(expected.payload_contents_length);

		const payload = decrypted.payload_contents[0];

		expect(payload.payload_content_header_hash.equals(expected.payload_content_header_hash)).toBe(true);
		expect(payload.payload_content_header_hash_signature.equals(expected.payload_content_header_hash_signature)).toBe(true);
		expect(payload.program_id).toBe(expected.program_id);
		expect(payload.content_datatype).toBe(expected.content_datatype);
		expect(payload.ns_data_id).toBe(expected.ns_data_id);
		expect(payload.content.equals(expected.content)).toBe(true);
	});
});
